...
import { MoreHorizontal, Pencil, Trash2, PartyPopper, Calendar } from "lucide-react";
...
import { deleteEvent } from "@/lib/actions/tourism";
import { TourismEventDialog } from "./tourism-event-dialog";
import type { Database } from "@/types/supabase";

type Tourism = Database["public"]["Tables"]["tourism"]["Row"];

interface TourismEventsTableProps {
  events: Tourism[];
}

export function TourismEventsTable({ events }: TourismEventsTableProps) {
  const [deleteDialogOpen, setDeleteDialogOpen] = useState(false);
  const [selectedEvent, setSelectedEvent] = useState<Tourism | null>(null);
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="flex items-center justify-end gap-2">
          <TourismEventDialog mode="edit" initialData={row.original}>
  ...
  if (events.length === 0) {
    return (
      <EmptyState
        ...
        description="Add your first festival or event to showcase Monkayo's celebrations"
      />
    );
  }
