import { Suspense } from "react";
import { Plus } from "lucide-react";
import { getAllDepartments } from "@/lib/actions/directory";
import { PageHeader } from "@/components/cms/page-header";
import { ContentCard } from "@/components/cms/content-card";
import { LoadingState } from "@/components/cms/loading-state";
import { Button } from "@/components/ui/button";
import { DepartmentsTable } from "./departments-table";
import { DepartmentDialog } from "./department-dialog";

export const metadata = {
  title: "Departments | LGU Monkayo CMS",
  description: "Manage municipal departments",
};

export default async function DepartmentsPage() {
  const departments = await getAllDepartments();

  return (
    <div className="flex flex-col min-h-screen">
      <PageHeader
        title="Departments"
        description="Manage municipal department directory and contact information"
        breadcrumbs={[
          { label: "Dashboard", href: "/dashboard" },
          { label: "Directory", href: "/content/directory/departments" },
          { label: "Departments" },
        ]}
        actions={
          <DepartmentDialog mode="create">
            <Button size="sm" className="bg-amber-600 hover:bg-amber-700 text-white dark:bg-amber-600 dark:hover:bg-amber-700">
              <Plus className="h-4 w-4 mr-1.5" />
              Add Department
            </Button>
          </DepartmentDialog>
        }
      />

      <main className="flex-1 p-4 lg:p-6">
        <ContentCard noPadding>
          <Suspense fallback={<LoadingState rows={5} className="p-6" />}>
            <DepartmentsTable departments={departments} />
          </Suspense>
        </ContentCard>
      </main>
    </div>
  );
}
