import { Suspense } from "react";
import { Plus } from "lucide-react";
import { getAllElectedOfficials } from "@/lib/actions/about";
import { PageHeader } from "@/components/cms/page-header";
import { ContentCard } from "@/components/cms/content-card";
import { LoadingState } from "@/components/cms/loading-state";
import { Button } from "@/components/ui/button";
import { OfficialsTable } from "./officials-table";
import { OfficialDialog } from "./official-dialog";

export const metadata = {
  title: "Elected Officials | LGU Monkayo CMS",
  description: "Manage elected officials information",
};

export default async function OfficialsPage() {
  const officials = await getAllElectedOfficials();

  return (
    <div className="flex flex-col min-h-screen">
      <PageHeader
        title="Elected Officials"
        description="Manage mayor, vice mayor, councilors, and other elected officials"
        breadcrumbs={[
          { label: "Dashboard", href: "/dashboard" },
          { label: "About Monkayo", href: "/content/about/officials" },
          { label: "Officials" },
        ]}
        actions={
          <OfficialDialog mode="create">
            <Button size="sm" className="bg-amber-600 hover:bg-amber-700 text-white dark:bg-amber-600 dark:hover:bg-amber-700">
              <Plus className="h-4 w-4 mr-1.5" />
              Add Official
            </Button>
          </OfficialDialog>
        }
      />

      <main className="flex-1 p-4 lg:p-6">
        <ContentCard noPadding>
          <Suspense fallback={<LoadingState rows={5} className="p-6" />}>
            <OfficialsTable officials={officials} />
          </Suspense>
        </ContentCard>
      </main>
    </div>
  );
}
